<script setup>
  ...
  const canSubmit = computed(() => {
    return loginForm.identifier.trim() && loginForm.password && !identifierError.value && !passwordError.value
  })
        ...
        <div class="w-full max-w-lg bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl border border-gray-100 dark:border-zinc-800 overflow-hidden relative">
          ...
          <div class="p-8 md:p-10">
            <div class="text-center mb-8">
              <h2 class="text-3xl font-black font-poppins dark:text-[#facc15] text-gray-900 mb-2">Login</h2>
              <p v-if="route.query.type === 'personal'" class="mt-5 text-gray-500 dark:text-gray-400 text-sm">단 하나의 명함으로 수많은 기업의 제안을 받아보세요.</p>
              <p v-else-if="route.query.type === 'business'" class="mt-5 text-gray-500 dark:text-gray-400 text-sm">인재를 만나는 가장 스마트한 방법.</p>
            </div>
            ...
            <form class="space-y-6" :key="formKey" autocomplete="off" @submit.prevent="login">
              <div class="space-y-2">
                ...
                />
                <p v-if="identifierError" class="text-xs font-bold text-red-500">{{ identifierError }}</p>
              </div>
            ...
            <p class="text-center mt-6 text-sm text-gray-400">
              로그인 정보를 잊어버리셨나요?
              <router-link 
                :to="route.query.type === 'business' ? '/find-business-account' : '/find-personal-account'" 
