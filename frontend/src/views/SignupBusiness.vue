<script setup>
    ...
    const canProceed = computed(() => {
      return rawBusinessNumber.value.length === 10 && 
    ...
    <template>
      <div class="business-signup-container min-h-screen justify-center bg-slate-50 bg-white text-slate-900 dark:bg-zinc-950/70 dark:text-white flex flex-col items-center py-12 px-4 transition-colors">
    
        <!-- 메인 카드 -->
        <div class="w-full max-w-lg rounded-3xl shadow-2xl p-8 md:p-10 border transition-all bg-white border-slate-100 text-slate-900 dark:bg-zinc-900 dark:border-zinc-800 dark:text-white">
          <form @submit.prevent="handleSubmit" class="space-y-8">
            
            <!-- 상단 로고 및 헤더 -->
            <div class="mb-10 text-center">
                <h2 class="text-4xl font-bold dark:text-[#facc15] text-black mb-2 font-poppins">Join Poticard</h2>
                <p class="text-sm mt-2 text-slate-500 dark:text-zinc-400">인재 채용을 위한 기업 인증 단계입니다.</p>
            </div>
            ...
            <section class="space-y-5">
              ...
              <div class="space-y-2">
                ...
                <div class="flex gap-2">
                  <input 
                    ...
                    :class="{
                      'border-red-500 ring-1 ring-red-500': !isBusinessNumberFocused && touched.businessNumber && rawBusinessNumber.length < 10,
                      'dark:border-[#facc15] dark:ring-[#facc15]': !isBusinessNumberFocused && touched.businessNumber && rawBusinessNumber.length < 10
                    }"
                    maxlength="12"
                    @focus="isBusinessNumberFocused = true"
                    @blur="isBusinessNumberFocused = false; touched.businessNumber = true"
                  >
                  <button 
                    type="button" 
                ...
                <p v-if="isDuplicateChecked && rawBusinessNumber.length === 10" class="text-xs ml-1 mt-1 font-medium text-blue-500 dark:text-blue-400">
                ...
                <p v-if="touched.businessNumber && rawBusinessNumber.length < 10" 
                  ...
                  <i class="fa-solid fa-circle-exclamation mr-1"></i> 사업자 번호 10자리를 정확히 입력해주세요.
                </p>
              </div>
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <i class="fa-solid fa-cloud-arrow-up text-2xl mb-2 transition-colors text-slate-400 group-hover:text-[#facc15] dark:text-zinc-500 dark:group-hover:text-[#facc15]"></i>
                  <p class="text-sm font-medium text-slate-500 dark:text-zinc-400" v-if="!fileName">파일을 드래그하거나 클릭하여 업로드</p>
                  <p class="text-sm font-bold text-slate-900 dark:text-[#facc15]" v-else>{{ fileName }}</p>
                  <p class="text-[11px] font-poppins mt-1 text-slate-400 dark:text-zinc-500">PDF, JPG, PNG (MAX 5MB)</p>
                </div>
                <p v-if="touched.file && !form.file" 
                   class="text-xs ml-1 mt-1 font-medium text-red-500 dark:text-[#facc15]">
          ...
          <p class="mt-8 text-center text-sm text-zinc-400">
            ...
            <router-link 
              ...
              class="font-bold hover:underline text-[#facc15]"
            >
              로그인
            </router-link>
          </p>
